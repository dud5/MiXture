...
type Generators =
    ...
    static member JSValueStringGen =
        gen {
            let! s = Arb.generate |> Gen.suchThat(containsNull >> not)
            return s
        }
...
type NEmbedding() = 
        ...
        member this.Object() =
            fsCheck "Records - check for valid" (Prop.forAll (Arb.fromGen Generators.JSValueRecGen)
                (fun (r:website,x:string) ->
                    let j = public_execute_string(x)
                    let back = public_project<website> j
                    r = back))
        ...
        member this.FSObject() =
            ...
            Assert.Throws<ProjectException>(fun () ->
                                                public_project<Website> j |> ignore)
            |> ignore